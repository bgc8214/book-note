...
한 줄: 돈을 쫓지 말라는 조언이 어떤 사람에게는 독이 된다는 데서 출발해, 슬럼프·정체기·소비를 다루는 관점을 정리한 에세이
...
- 0장 서문 — 평범한 나는 어떻게 천재들의 상을 받게 되었나 — 저자는 자라면서 "그런데 네가 천재는 아니잖니"라는 말을 아버지에게 반복해서 들었고, 훗날 천재들의 상이라 불리는 맥아더상을 받았다. → `books/grit/ch00-preface.md`
- 1장 제1부 그릿이란 무엇인가 — 1부는 재능 신화를 데이터로 흔든다. → `books/grit/ch01-what-is-grit.md`
- 2장 제2부 '포기하지 않는 나'는 어떻게 만들어지는가 — 2부는 그릿을 스스로 기르는 순서를 넷으로 제시한다. → `books/grit/ch02-growing-grit-inside.md`
- 3장 제3부 내면이 강한 아이는 어떻게 길러지는가 — 3부는 방향을 바꿔 밖에서 안으로 간다. → `books/grit/ch03-growing-grit-outside.md`
- 용어집 → `books/grit/glossary.md`
